...
if __name__ == "__main__":
    ...
    working_directory = tempfile.mkdtemp(dir="d:\\gitmp")

    project = Project("https://github.com/radsz/jacop") #Project(args.url)
    project.clone(working_directory)
    project.checkout_commit("8f09fd977a") #(args.commit)
    output = {
        "commit": project.get_commit(),
        "test_results": []
    }
    tmp = project.pom.get_included_excluded_tests()
    (includes, excludes) = (tmp[0],tmp[1])
    exclude_config = []
