...
class Project:
    ...
    def clone(self, path: str):
        try:
            cmd = "powershell.exe cd %s;git clone -q --depth=1 %s;" % (path, self.url)
    ...
    def checkout_commit(self, commit: str):
        try:
            cmd = 'powershell.exe cd %s; git fetch -q origin %s; git checkout -q %s' % (
    ...
    def get_commit(self):
        cmd = 'powershell.exe cd %s; git rev-parse HEAD' % (self.path)
    ...
    def clean(self):
        cmd = 'powershell.exe cd %s;mvn clean -B;' % (self.path)
        try:
            subprocess.check_call(cmd)
            return True
        except:
    ...
    def dependency_tree(self):
        ...
        cmd = "powershell.exe cd %s; mvn %s dependency:tree -Dverbose=true -DoutputFile=dependency_tree.txt -q;cat dependency_tree.txt;rm dependency_tree.txt" % (self.path, maven_proxy)
    ...
    def classpath(self):
        ...
        cmd = "powershell.exe cd %s; mvn %s dependency:build-classpath -Dscope=test;" % (
    ...
    def compile(self, clean: bool = True, stdout: str = None, timeout: str = None):
        ...
        if os.path.exists(maven_proxy_path):
            maven_proxy = '-gs %s' % (maven_proxy_path)
        cmd = 'powershell.exe cd %s;%s%s mvn %s test -DskipTests -e -Dgpg.skip=true -B -Dmaven.javadoc.skip=true -Drat.skip=true -Danimal.sniffer.skip=true -Dmaven.javadoc.skip=true -Dlicense.skip=true -Dsource.skip=true' % (
    ...
    def test(self, clean: bool = True, stdout: str = None, timeout: str = None):
        ...
        if os.path.exists(maven_proxy_path):
            maven_proxy = '-gs %s' % (maven_proxy_path)
        cmd = 'powershell.exe cd %s;%s%s mvn %s test -DtrimStackTrace=false -e --fail-never -Dmaven.test.failure.ignore=true -B -Dmaven.javadoc.skip=true -Drat.skip=true -Danimal.sniffer.skip=true -Dmaven.javadoc.skip=true -Dlicense.skip=true -Dsource.skip=true' % (
            self.path, clean_cmd, timeout_cmd, maven_proxy)
        if stdout is not None:
            cmd += ' > %s 2>&1' % (stdout)
        try:
    ...
    def install(self, stdout: str = None, timeout: str = None) -> bool:
        ...
        if os.path.exists(maven_proxy_path):
            maven_proxy = '-gs %s' % (maven_proxy_path)
        cmd = 'powershell.exe cd %s; %s mvn %s install -DskipTests -Dgpg.skip=true -Dmaven.test.error.ignore=true -Dmaven.test.failure.ignore=true -B -Dmaven.javadoc.skip=true -Drat.skip=true -Danimal.sniffer.skip=true -Dmaven.javadoc.skip=true -Dlicense.skip=true -Dsource.skip=true' % (
            self.path,
            timeout_cmd,
            maven_proxy)
        if stdout is not None:
    ...
    def package(self, stdout: str = None, timeout: str = None) -> bool:
        ...
        if os.path.exists(maven_proxy_path):
            maven_proxy = '-gs %s' % (maven_proxy_path)
        cmd = 'powershell.exe cd %s; mvn clean -q -B;%s mvn %s package -e --fail-never -Dgpg.skip=true  -Dmaven.test.error.ignore=true -Dmaven.test.failure.ignore=true -B -Dmaven.javadoc.skip=true -Drat.skip=true -Danimal.sniffer.skip=true -Dmaven.javadoc.skip=true -Dlicense.skip=true -Dsource.skip=true' % (
            self.path,
            timeout_cmd,
            maven_proxy)
        if stdout is not None:
    ...
    def copy_jar(self, dst: str) -> bool:
        ...
        cmd = 'powershell.exe cd %s/target; cp *jar-with-dependencies.jar %s;' % (
            self.path, dst)
        try:
            subprocess.check_call(cmd)
            return True
    ...
    def copy_test_results(self, dst: str):
        ...
        if not os.path.exists(os.path.join(self.path, "target", "surefire-reports")):
            return False
        cmd = 'powershell.exe cd %s/target/; cp -r surefire-reports %s' % (self.path, dst)
        print(cmd)
        try:
            subprocess.check_call(cmd)
            return True
    ...
    def copy_jacoco(self, dst: str):
        dst = os.path.abspath(dst)
        cmd = 'powershell.exe cd %s/target/; cp -r site/jacoco/jacoco.xml %s; cp -r site/jacoco/jacoco.csv %s' % (
            self.path, dst, dst)
        try:
            subprocess.check_call(cmd)
            return True
    ...
    def copy_pom(self, dst: str):
        dst = os.path.abspath(dst)
        cmd = 'powershell.exe cd %s; cp -r %s %s' % (self.path, self.pom.poms[0]['path'], dst)
        try:
            subprocess.check_call(cmd)
            return True
        except:
    ...
    def copy_report(self, dst: str):
        dst = os.path.abspath(dst)
        cmd = f'powershell.exe cd {self.path}; mv .jdbl/*-debloated.jar .jdbl/debloat.jar; mv .jdbl/*-original.jar .jdbl/original.jar; cp -r .jdbl/* {dst};'
        try:
            subprocess.check_call(cmd)
            return True
        except:
    ...
    def unzip_debloat(self, root: str, library: str, version: str, debloated: bool = True):
        ...
        cmd = "powershell.exe mkdir -p %s/target/classes/;powershell.exe cd %s/target/classes/; jar xf %s;" % (
    ...
    def inject_debloat_library(self, root: str, library: str, version: str, debloated: bool = True):
        ...
        cmd = "powershell.exe cd %s; mvn install:install-file -Dfile=%s -DgroupId=%s -DartifactId=%s -Dversion=%s -Dpackaging=jar -B -Dmaven.javadoc.skip=true;" % (
